...
const char *k_versionStr = "v26.03.04";
...
bool Config::writeRegistryToJSON()
{
   ...
   for ( int i = 0; k_keyMappings[ i ].flatKey != nullptr; i++ )
   {
      ...
      {
         TVMG_DEBUG( "Ignoring empty %s (v=%s) (vol=%s)",entry.key,entry.value,entry.volatileValue );
         continue;
      }
      ...
      if ( length > 1 && length < 127 )
      {
         ...
         if ( ( line [ 0 ] != '#' && line [ 0 ] != '/') )
         {
            ...
            if ( sscanf( line,"%s %s",keyVal.key,keyVal.value ) == 2 )
            {
               ...
               setRegistryEntry( keyVal.key,keyVal.value,false );
               numLines++;
            }
         }
      }
...
void  Config::setRegistryEntry( const char *key,const char *value, bool isVolatile )
{
   ...
   char *currentValue = entry.value;
   if ( isVolatile )
   {
      currentValue = entry.volatileValue;
   }
   else if ( strlen( entry.volatileValue ) )
   {
      entry.volatileValue[ 0 ] = '\0';
   }
...
const char *Config::getRegistryString( const char *key )
{
   ...
   else
   {
      ...
      KeyValue &entry = s_instance->m_entries[ index ];
      if ( strlen( entry.volatileValue ) )
      {
         return entry.volatileValue;
      }
      return entry.value;
   }
}
