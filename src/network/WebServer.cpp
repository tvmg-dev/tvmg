...
#include "src/network/html/opensource_html.h"
#include "src/network/html/reboot_html.h"
#include "src/network/html/status_html.h"
...
String processor(const String& var)
{
  ...
  if(var == "SAVE_PATH_INPUT")
  {
   ...
   if(var == "GFX_LIBRARY")
   {
      ...
      return "<tr><td>U8g2</td><td>BSD 3-Clause</td><td>2.35.30</td></tr>";
#else
      return "";
#endif
   }
...
void WebServer::setupMiscHandlers()
{
   ...
   m_webServer->on("/opensource", HTTP_GET, [this](AsyncWebServerRequest *request)
   {
      AUTHENTICATE;

      request->send_P(200, "text/html", credits_html, processor);
   });
