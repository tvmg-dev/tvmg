...
public:
   ...
   void initialise();
   void sample();
   static bool isValidSensor( uint8_t id );
   static bool getTemperature( uint8_t shellyId,uint8_t tempId,float *val );
   ShellyPowerSensor *readNextSensor( uint8_t index );
   bool getPower( uint8_t index );

private:
   enum ShellyModel { UNKNOWN,PMG3,PM1G3,EM,EMG3 };

   /// single temperature reading from a Shelly device
   struct ShellyTemp {
      uint8_t m_tempId;
      float   m_value;
   };
   ...
   typedef struct {
      ...
      std::vector<ShellyTemp> m_temps; // temperature values if in the response
      bool              m_isValid;
   } PrivateSensor;
